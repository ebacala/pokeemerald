...
EWRAM_DATA static bool8 sAreStartersRandomized = false;

// Base form Pokemon species array
static const u16 sBaseFormPokemon[189] = {
    SPECIES_BULBASAUR,
    SPECIES_CHARMANDER,
    SPECIES_SQUIRTLE,
    SPECIES_CATERPIE,
...
EWRAM_DATA static u16 sRandomizedStarterMon[STARTER_MON_COUNT] = {0};

static void InitRandomStarterMons(void)
{
    int i;
    for (i = 0; i < STARTER_MON_COUNT; i++)
    {
        sRandomizedStarterMon[i] = sBaseFormPokemon[Random() % ARRAY_COUNT(sBaseFormPokemon)];
    }
}
...
void RandomizeStarters(void)
{
    sAreStartersRandomized = true;
    InitRandomStarterMons();
}
...
u16 GetStarterPokemon(u16 chosenStarterId)
{
    ...
    if (sAreStartersRandomized)
        return sRandomizedStarterMon[chosenStarterId];
